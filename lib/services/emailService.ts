...
// 1. Updated Email Service with Simple HTML - lib/services/emailService.ts
import nodemailer from 'nodemailer';
...
export class EmailService {
  ...
  // Simple birthday email with basic HTML
  async sendBirthdayNotificationEmail(to: string, username: string) {
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <p>Happy Birthday! We hope you have a wonderful day.</p>
        
        <p>You can now upgrade your account type to access new features:</p>
        <ul>
        ...
        <p><a href="${process.env.FRONTEND_URL || 'http://localhost:3000'}/dashboard" 
              style="background-color: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">
           Visit Dashboard
        </a></p>
        
        <p>Best regards,<br>BudgetMate Team</p>
      </div>
    `;
  ...
  // Simple payment reminder email with basic HTML
  async sendPaymentReminderEmail(
    to: string, 
    username: string, 
    reminderDetails: {
    ...
    if (daysUntilDue > 0) {
      ...
      messageText = `Your payment is due in ${daysUntilDue} day(s).`;
      urgencyColor = '#28a745';
    } else if (daysUntilDue === 0) {
      subject = `🚨 Payment Due Today: ${title}`;
      messageText = `Your payment is due today!`;
      urgencyColor = '#ffc107';
    } else {
      subject = `⚠️ Overdue Payment: ${title}`;
      messageText = `Your payment is overdue by ${Math.abs(daysUntilDue)} day(s).`;
      urgencyColor = '#dc3545';
    }
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <p><a href="${process.env.FRONTEND_URL || 'http://localhost:3000'}/dashboard/payment-reminders" 
              style="background-color: ${urgencyColor}; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">
           View Dashboard
        </a></p>
